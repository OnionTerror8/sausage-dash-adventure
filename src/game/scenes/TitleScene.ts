...
export class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      this.tweens.add({ targets: c, y: c.y + 12, yoyo: true, repeat: -1, duration: 2200 + i * 300, ease: "sine.inOut" });
    }
    ...
    // Title
    const title = this.add.text(width / 2, 130, "Sausage Dash!", {
    ...
    this.tweens.add({ targets: title, scale: { from: 1, to: 1.06 }, yoyo: true, repeat: -1, duration: 900, ease: "sine.inOut" });

    // Sausage character preview
    const char = this.add.container(width / 2, height / 2 + 20);
    drawSausage(this, char, save.equipped);
    this.tweens.add({ targets: char, y: char.y - 18, yoyo: true, repeat: -1, duration: 700, ease: "sine.inOut" });
    ...
    // Shop button
    this.makeButton(width / 2 - 200, height - 50, 180, 60, 0x5fd0ff, "Shop", () => {
      SFX.click();
      this.scene.start("Shop");
    }, 26);

    // Settings — sound toggle
    this.makeButton(width / 2 + 200, height - 50, 180, 60, 0xffcf3a, save.settings.sound ? "Sound On" : "Sound Off", () => {
      SFX.click();
      const d = updateSave((d) => { d.settings.sound = !d.settings.sound; });
    ...
    this.makeButton(60, height / 2, 60, 60, 0xffffff, "‹", () => {
      ...
      updateSave((d) => { d.equipped.theme = next.id; });
      this.scene.restart();
    }, 36, 0x333333);
    this.makeButton(width - 60, height / 2, 60, 60, 0xffffff, "›", () => {
      const owned = WORLDS.filter((w) => save.unlocked.includes(w.id));
      const i = owned.findIndex((w) => w.id === save.equipped.theme);
    ...
    this.add.text(width / 2, height / 2 - 120, WORLDS[themeIdx]?.name ?? "", {
    ...
    // Best score badge
    if (save.bestScore > 0) {
      this.add.text(width / 2, 220, `Best: ${save.bestScore}`, {
        fontFamily: "'Fredoka',system-ui,sans-serif", fontSize: "26px", color: "#ffffff",
        stroke: "#333", strokeThickness: 4,
      }).setOrigin(0.5);
    }
  }
  ...
  private makeButton(
    ...
    onClick: () => void, fontSize = 34, textColor = 0xffffff,
  ) {
